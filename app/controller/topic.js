...
class topicController extends Controller {
   ...
   * @response 200 querytopicResponse 角色列表
   */
  async index() {
   ...
   * @response 200 gettopicResponse 角色信息
   */
  async show() {
    const { ctx, service } = this;
    const { id } = ctx.params;
   ...
   * @description 创建角色
   * @router post /topics
   * @request body createtopicRequest *body
  ...
  async removes() {
    ...
    await service.topic.removes(payload);
    ctx.success();
  }
}
